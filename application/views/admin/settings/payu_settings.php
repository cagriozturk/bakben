...
<div class="row">
   <div class="col-md-8">
      ...
      <form method="POST" action="<?php echo base_url();?>admin/payu_settings" id="settings_form" >
         <?php if(count($data)>0)
            $data=$data[0];
            ?>
		...
		<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
         ...
         <div class="form-group">
            <label for="inputEmail">Tüccar Anahtarı</label>
            <?php 
               $val = '';
               	if ($this->input->post( 'merchant_key' )) {
               	$val = $this->input->post( 'merchant_key' );
               	}
               	elseif (count($data)) {
               		$val = $data->merchant_key; 
               	}
            ?>
            <input type="text" class="form-control" id="site_title" name="merchant_key" value="<?php echo $val;?>" placeholder="Merchant Key" >
         </div>
		 
		    <div class="form-group">
            <label for="inputEmail">Salt</label>
            <?php 
               $val = '';
               	if ($this->input->post( 'salt' )) {
               	$val = $this->input->post( 'salt' );
               	}
               	elseif (count($data)) {
               		$val = $data->salt; 
               	}
            ?>
            <input type="text" class="form-control" id="site_title" name="salt" value="<?php echo $val;?>" placeholder="Salt" >
         </div>
		 
		 <div class="form-group">
            <label for="inputEmail">Test URL</label>
            <?php 
               $val = '';
               	if ($this->input->post( 'test_url' )) {
               	$val = $this->input->post( 'test_url' );
               	}
               	elseif (count($data)) {
               		$val = $data->test_url; 
               	}
            ?>
            <input type="text" class="form-control" id="site_title" name="test_url" value="<?php echo $val;?>" placeholder="Test URL" >
         </div>
		
		
		 <div class="form-group">
            <label for="inputEmail">Canlı URL</label>
            <?php 
               $val = '';
               	if ($this->input->post( 'live_url' )) {
               	$val = $this->input->post( 'live_url' );
               	}
               	elseif (count($data)) {
               		$val = $data->test_url; 
               	}
            ?>
            <input type="text" class="form-control" id="site_title" name="live_url" value="<?php echo $val;?>" placeholder="Live URL" >
         </div>
