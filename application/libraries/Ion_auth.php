		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				else
				{
					...
					$admin_email = $this->base_model->run_query("SELECT site_title, smtp_user FROM ".$this->db->dbprefix('general_settings')." , ".$this->db->dbprefix('email_setting')." limit 1");
					$from_email='';
					$site_title='';
					if(count($admin_email)>0)
					{
						$from_email = $admin_email[0]->smtp_user;	
						$site_title = $admin_email[0]->site_title;	
					}
					
					$this->email->from($from_email,$site_title);
					$this->email->to($user->email);
					$this->email->subject($site_title . ' - ' . $this->lang->line('email_forgotten_password_subject'));
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			else
			{
				...
				$this->email->from($from_email,$site_title);
				$this->email->to($user->email);
				$this->email->subject($site_title . ' - ' . $this->lang->line('email_new_password_subject'));
